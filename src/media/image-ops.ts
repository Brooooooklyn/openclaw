...
export async function resizeToJpeg(params: {
  ...
  return await transformer
    ...
    .resize({
      ...
      fit: params.withoutEnlargement !== false ? 2 : 1, // 2 = Inside (preserving aspect ratio), 1 = Fill
    })
    .jpeg(params.quality);
}
...
export async function hasAlphaChannel(buffer: Buffer): Promise<boolean> {
  try {
    ...
    // @napi-rs/image provides colorType which includes alpha variants
    // La8 (1), Rgba8 (3), La16 (5), Rgba16 (7), Rgba32F (9)
    const alphaColorTypes = [1, 3, 5, 7, 9];
    return alphaColorTypes.includes(meta.colorType);
  } catch {
    return false;
  }
}
...
export async function resizeToPng(params: {
  ...
  return await transformer
    ...
    .resize({
      width: params.maxSide,
      height: params.maxSide,
      fit: params.withoutEnlargement !== false ? 2 : 1, // 2 = Inside, 1 = Fill
    })
    .png({ compressionType });
}
